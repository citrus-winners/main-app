...
class AtService {
  ...
  factory AtService.getInstance() => _singleton;
...
void main() {
  MethodChannel _methodChannel = MethodChannel('atbridge');
  var backend = AtService.getInstance();
  _methodChannel?.setMethodCallHandler((call) {
    if (call.method == 'getCurrentUser') {
      return backend.get(call.arguments);
    } else if (call.method == 'login') {
      return backend.authenticate(call.arguments);
    } else if (call.method == 'registerRider') {
      return backend.put(call.arguments[0], call.arguments[1]);
    } else {
      return null;
    }
  });
